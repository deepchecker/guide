...
Будьте внимательны во время установки. В инструкции часто указываются относительные пути, поэтому - выполняйте команды в той последовательности, которой они указаны. Если вы не общаетесь с консолью на "Ты" - не перемещайтесь между папками, пока не завершите установку. Важно, чтобы вы выполняли указанные в данной инструкции команды, находясь в нужной папке.
...
- Заходим в админку чекера по ссылке **/options**, заполняем поля **'Имя бота (указываем без @)'** и **'Токен'**, жмем кнопку **'Привязать вебхук Telegram'**.  
